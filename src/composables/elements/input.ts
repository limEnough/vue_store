import { PropType, computed, ref, toRefs, useAttrs } from 'vue';
import { CountType } from '@/utils/string';
import { InputStates } from '@/constants/ui-constants';
import { CustomEmit } from '@/types/common.types';

type Emits = 'update:modelValue' | 'search';
type Charset = 'utf-8' | 'euc-kr' | string;

interface Props {
  modelValue: number | string | symbol | null;
  disabled: boolean;
  required: boolean;
...
export default function inputComposables(emit: CustomEmit<Emits>, props: Props) {
  ...
  const handleInput = (e: Event) => {
    ...
    if (sanitizedValue.length > Number(props.maxlength)) {
      e.target.value = lastValidValue.value as string; // 이전 유효값으로 재설정
